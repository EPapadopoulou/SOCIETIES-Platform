		...
		try {
			...
			for (RequestItem item : policy.getRequestItems()){
				...
				if (dataType.equalsIgnoreCase(CtxAttributeTypes.LOCATION_SYMBOLIC)){
					...
					BooleanRange accessHeldBooleanRange = new BooleanRange(booleanValues[1], booleanValues[1], ConditionConstants.RIGHT_TO_ACCESS_HELD_DATA); //no-yes
					...
					shareData.setValue(shareValues[0]);
					conditions.add(shareData);
					ShareDataRange shareDataRange = new ShareDataRange(shareValues[0], shareValues[1]);//no sharing - affiliated

					Condition mayInfer = new Condition();
					mayInfer.setConditionConstant(ConditionConstants.MAY_BE_INFERRED);
					mayInfer.setValue(booleanValues[1]);
					conditions.add(mayInfer);
					BooleanRange mayInferBooleanRange = new BooleanRange(booleanValues[1], booleanValues[1], ConditionConstants.MAY_BE_INFERRED);// no-yes
