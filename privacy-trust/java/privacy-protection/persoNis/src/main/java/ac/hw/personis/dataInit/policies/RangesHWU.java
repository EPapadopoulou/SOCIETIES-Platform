		...
		try {
			...
			for (RequestItem item : policy.getRequestItems()){
				...
				if (dataType.equalsIgnoreCase(CtxAttributeTypes.LOCATION_SYMBOLIC)){
					...
					DataRetentionRange dataRetentionRange = new DataRetentionRange(dataRetentionValues[0], dataRetentionValues[0]); //1h - 12h
					...
					conditions.add(storeSecure);
					BooleanRange storeSecureBooleanRange = new BooleanRange(booleanValues[0], booleanValues[0], ConditionConstants.STORE_IN_SECURE_STORAGE); //no

					Condition access = new Condition();
					access.setConditionConstant(ConditionConstants.RIGHT_TO_ACCESS_HELD_DATA);
					access.setValue(booleanValues[0]);
					conditions.add(access);
					BooleanRange accessHeldBooleanRange = new BooleanRange(booleanValues[0], booleanValues[0], ConditionConstants.RIGHT_TO_ACCESS_HELD_DATA); //no-yes

					Condition correct = new Condition();
					correct.setConditionConstant(ConditionConstants.RIGHT_TO_CORRECT_INCORRECT_DATA);
					correct.setValue(booleanValues[0]);
					conditions.add(correct);
					BooleanRange correctDataBooleanRange = new BooleanRange(booleanValues[0], booleanValues[0], ConditionConstants.RIGHT_TO_CORRECT_INCORRECT_DATA);
					...
					storeSecure.setValue(booleanValues[1]);
					conditions.add(storeSecure);
					BooleanRange storeSecureBooleanRange = new BooleanRange(booleanValues[1], booleanValues[0], ConditionConstants.STORE_IN_SECURE_STORAGE); //no
